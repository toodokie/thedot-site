...
function request(overrides: Partial<ContentRequestRow> = {}): ContentRequestRow {
  return {
    ...
    base_version: 3, payload: {}, status: 'applied', requester_name: 'Maria',
...
describe('reReviewContext', () => {
  it('labels a released version built from resolved client edit feedback', () => {
    expect(reReviewContext(4, 'needs_review', [request(), request({ id: 'request-2', status: 'superseded' })]))
      .toEqual({ previousVersion: 3, changeCount: 2 })
  })
