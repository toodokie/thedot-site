-- Let an approved Agency Ops safe-merge candidate become the exact reconciliation
-- boundary for a bundled client edit. The raw client proposal remains immutable.
...
begin
  if pg_catalog.to_regprocedure(
       'public.portal_core_mark_content_request_bundle_prepared(uuid[],text,text,uuid)'
     ) is null
     or pg_catalog.to_regclass('public.content_request_review_candidates') is null
     or pg_catalog.to_regprocedure('public.assert_portal_security()') is null then
    raise exception '0080 requires the reviewed bundle and cumulative portal security system';
  end if;
end;
$$;

select public.assert_portal_security();
alter function public.assert_portal_security() rename to assert_portal_slice79_security;
revoke all on function public.assert_portal_slice79_security() from public, anon, authenticated;
grant execute on function public.assert_portal_slice79_security() to service_role;

create or replace function public.portal_core_mark_content_request_bundle_prepared(
...
begin
  ...
  select pg_catalog.count(*) into v_review_count
  from public.content_request_review_candidates c
  where c.request_id = any(v_ids);
  if v_review_count not in (0, pg_catalog.array_length(v_ids, 1)) then
    raise exception 'reviewed bundle must include a candidate for every request';
  end if;
  if v_review_count > 0 and exists (
    select 1 from public.content_request_review_candidates c
    where c.request_id = any(v_ids) and c.status <> 'approved'
  ) then
    raise exception 'reviewed bundle candidates must all be approved';
  end if;

  for v_request in
    select * from public.content_change_requests r where r.id = any(v_ids) order by r.id for update
  loop
    ...
    where block.value->>'key' = v_request.payload->>'block_key';
    select c.candidate_text into v_review_body
    from public.content_request_review_candidates c
    where c.request_id = v_request.id and c.status = 'approved';

    v_working_body := pg_catalog.btrim(pg_catalog.replace(pg_catalog.replace(
      v_working_body, E'\r\n', E'\n'), E'\r', E'\n'));
    v_proposed_body := pg_catalog.btrim(pg_catalog.replace(pg_catalog.replace(
      coalesce(v_review_body, v_request.payload->>'proposed_text'), E'\r\n', E'\n'), E'\r', E'\n'));
    if v_base_body is null or v_working_body is null or v_proposed_body is null
       ...
       or not (
         v_working_body = v_proposed_body
         or (
           v_review_body is null
           and pg_catalog.char_length(v_proposed_body) >= 20
           and (
             (pg_catalog.left(v_working_body, pg_catalog.char_length(v_proposed_body)) = v_proposed_body
               and pg_catalog.substr(v_working_body, pg_catalog.char_length(v_proposed_body) + 1, 1) in (' ', E'\n'))
             or pg_catalog.strpos(v_working_body, E'\n' || v_proposed_body || ' ') > 0
             or pg_catalog.strpos(v_working_body, E'\n' || v_proposed_body || E'\n') > 0
             or pg_catalog.right(v_working_body, pg_catalog.char_length(v_proposed_body) + 1) = E'\n' || v_proposed_body
           )
         )
       ) then
      raise exception 'bundle request does not exactly match the synced copy as an approved candidate, exact block or leading segment';
    end if;
  end loop;
...
create function public.assert_portal_reviewed_bundle_security()
returns void language plpgsql security definer set search_path='' as $$
declare v_core_def text;
begin
  select pg_catalog.pg_get_functiondef(
    'public.portal_core_mark_content_request_bundle_prepared(uuid[],text,text,uuid)'::pg_catalog.regprocedure
  ) into v_core_def;
  if v_core_def is null
     or v_core_def not ilike '%content_request_review_candidates%'
     or v_core_def not ilike '%reviewed bundle candidates must all be approved%'
     or v_core_def not ilike '%v_review_body is null%'
     or v_core_def not ilike '%original_checksum%'
     or v_core_def not ilike '%for update%'
     or v_core_def not ilike '%portal_command_receipts%' then
    raise exception 'reviewed bundle reconciliation is incomplete';
  end if;
  if pg_catalog.has_function_privilege(
       'anon','public.portal_core_mark_content_request_bundle_prepared(uuid[],text,text,uuid)','EXECUTE'
     )
     or pg_catalog.has_function_privilege(
       'authenticated','public.portal_core_mark_content_request_bundle_prepared(uuid[],text,text,uuid)','EXECUTE'
     )
     or pg_catalog.has_function_privilege(
       'service_role','public.portal_core_mark_content_request_bundle_prepared(uuid[],text,text,uuid)','EXECUTE'
     ) then
    raise exception 'reviewed bundle core grants are unsafe';
  end if;
  if pg_catalog.has_function_privilege(
    ...
    raise exception 'reviewed bundle boundary grants are unsafe';
  end if;
end;
$$;

revoke all on function public.assert_portal_reviewed_bundle_security()
  from public, anon, authenticated;
grant execute on function public.assert_portal_reviewed_bundle_security() to service_role;

create or replace function public.assert_portal_security()
returns void language plpgsql security definer set search_path='' as $$
begin
  perform public.assert_portal_slice79_security();
  perform public.assert_portal_reviewed_bundle_security();
end;
$$;
