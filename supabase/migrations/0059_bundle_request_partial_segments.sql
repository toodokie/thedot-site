-- Portal edit requests can contain one edited sentence from a larger copy block.
-- Permit that exact sentence as an explicitly reviewed leading segment while
-- retaining the existing exact-block path and all reconciliation invariants.
begin;
...
begin
  if pg_catalog.to_regprocedure('public.mark_content_request_bundle_prepared(uuid[],text,text,uuid)') is null
    ...
    raise exception '0059 requires bundled content request reconciliation';
  end if;
end;
$$;

select public.assert_portal_security();
alter function public.assert_portal_security() rename to assert_portal_slice57_security;
revoke all on function public.assert_portal_slice57_security() from public, anon, authenticated;
grant execute on function public.assert_portal_slice57_security() to service_role;
  ...
  for v_request in select * from public.content_change_requests r where r.id = any(v_ids) order by r.id for update loop
    ...
    if v_base_body is null or v_working_body is null or v_proposed_body is null
       ...
       or not (
         ...
         or (
           pg_catalog.char_length(v_proposed_body) >= 20
           and (
      ...
      raise exception 'bundle request does not exactly match the synced copy as an exact block or leading segment';
    end if;
  end loop;
...
create function public.assert_portal_bundle_partial_segments_security()
...
begin
  ...
  if v_def is null
     or v_def not ilike '%exact block or leading segment%'
     or v_def not ilike '%char_length(v_proposed_body) >= 20%'
     or v_def not ilike '%original_checksum%'
     or v_def ilike '%regexp_replace(v_working_body%' then
    raise exception 'content request partial-segment reconciliation drifted';
  end if;
...
$$;
revoke all on function public.assert_portal_bundle_partial_segments_security() from public, anon, authenticated;
grant execute on function public.assert_portal_bundle_partial_segments_security() to service_role;

create or replace function public.assert_portal_security()
returns void language plpgsql security definer set search_path='' as $$
begin
  perform public.assert_portal_slice57_security();
  perform public.assert_portal_bundle_partial_segments_security();
end;
$$;
