-- One Maria review can contain several edits to distinct blocks of one released version.
-- Reconcile them as ONE canonical revision, then close every exact request together. The
-- previous 0047 guard allowed the revision to start, but the single-request prepare writer
-- could only attach one request to the resulting version and left its siblings stranded.
begin;

do $$
begin
  if pg_catalog.to_regprocedure('public.assert_portal_security()') is null
     or pg_catalog.to_regprocedure('public.mark_content_request_prepared(uuid,text,text,uuid)') is null
     or pg_catalog.to_regclass('public.canonical_change_jobs') is null then
    raise exception '0056 requires the content-request reconciliation boundary';
  end if;
end;
$$;

select public.assert_portal_security();
alter function public.assert_portal_security() rename to assert_portal_slice46_security;
revoke all on function public.assert_portal_slice46_security() from public, anon, authenticated;
grant execute on function public.assert_portal_slice46_security() to service_role;

-- 0014 assumed one request could produce one version. A bundled review intentionally
-- produces several prepared/applied edit rows for the same immutable version, so retain
-- the lookup without retaining an invalid uniqueness rule.
drop index public.content_change_requests_canonical_result;
create index content_change_requests_canonical_result_lookup
...
begin
  ...
  for v_request in select * from public.content_change_requests r where r.id = any(v_ids) order by r.id for update loop
    ...
    if v_base_body is null or v_working_body is null
       ...
       or pg_catalog.btrim(pg_catalog.regexp_replace(v_working_body, E'\\r\\n?', E'\\n', 'g'))
          is distinct from pg_catalog.btrim(pg_catalog.regexp_replace(v_request.payload->>'proposed_text', E'\\r\\n?', E'\\n', 'g')) then
...
create function public.assert_portal_request_bundle_security()
returns void language plpgsql security definer set search_path='' as $$
declare v_def text;
begin
  if not exists (select 1 from pg_catalog.pg_indexes i
    where i.schemaname='public' and i.indexname='content_change_requests_canonical_result_lookup'
      and i.indexdef not ilike '%unique%') then
  ...
  if v_def is null or v_def not ilike '%security definer%'
     ...
     or v_def not ilike '%bundle must have exactly one applying lead request%'
     or v_def not ilike '%bundle request does not exactly match the synced copy%'
     or v_def not ilike '%for update%' then
    raise exception 'content request bundle writer drifted';
  end if;
...
$$;
revoke all on function public.assert_portal_request_bundle_security() from public, anon, authenticated;
grant execute on function public.assert_portal_request_bundle_security() to service_role;

create or replace function public.assert_portal_security()
returns void language plpgsql security definer set search_path='' as $$
begin
  perform public.assert_portal_slice46_security();
  perform public.assert_portal_request_bundle_security();
end;
$$;
