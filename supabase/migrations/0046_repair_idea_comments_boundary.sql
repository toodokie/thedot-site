-- Repair migration-history drift: production recorded 0041 but did not contain any
-- of its objects. Reapply the complete, hardened boundary so later migrations cannot
-- silently run without the idea-comments surface.
...
begin
  if pg_catalog.to_regprocedure('public.assert_portal_security()') is null
    ...
    raise exception '0046 requires ideas, command receipts, inbox, notifications, and activity vocabulary';
  end if;
end;
$$;

select public.assert_portal_security();
alter function public.assert_portal_security() rename to assert_portal_slice39_security;
revoke all on function public.assert_portal_slice39_security() from public, anon, authenticated;
grant execute on function public.assert_portal_slice39_security() to service_role;
...
create index if not exists idea_comments_by_idea on public.idea_comments(idea_id, created_at, id);

alter table public.idea_comments enable row level security;
drop policy if exists idea_comments_read on public.idea_comments;
...
begin
  ...
  select i.* into v_idea from public.content_ideas i
  join public.client_users cu on cu.client_id = i.client_id and cu.auth_user_id = v_uid
  where i.id = p_idea_id for update of i;
  ...
  select nullif(pg_catalog.btrim(cu.name), '') into v_actor from public.client_users cu
  where cu.client_id = v_idea.client_id and cu.auth_user_id = v_uid limit 1;
  ...
  if found then
    if v_receipt.command_type <> 'add_idea_comment' or v_receipt.request_fingerprint <> v_fingerprint then
...
$$;
revoke all on function public.add_idea_comment(uuid,text,uuid) from public, anon, authenticated, service_role;
grant execute on function public.add_idea_comment(uuid,text,uuid) to authenticated;

create or replace function public.add_agency_idea_comment(
  p_idea_id uuid,
  p_body text,
  p_actor_key text,
  p_idempotency_key uuid
...
begin
  ...
  where r.client_id = v_idea.client_id and r.idempotency_key = p_idempotency_key::text;
  if found then
    if v_receipt.command_type <> 'add_agency_idea_comment' or v_receipt.request_fingerprint <> v_fingerprint then
      raise exception 'idempotency key reused with different request';
    end if;
    return v_receipt.response;
  end if;
...
$$;
revoke all on function public.add_agency_idea_comment(uuid,text,text,uuid) from public, anon, authenticated, service_role;
grant execute on function public.add_agency_idea_comment(uuid,text,text,uuid) to service_role;

create or replace function public.add_agency_idea_comment_reply(
...
$$;
revoke all on function public.add_agency_idea_comment_reply(uuid,text,text,uuid) from public, anon, authenticated, service_role;
grant execute on function public.add_agency_idea_comment_reply(uuid,text,text,uuid) to service_role;

create or replace function public.portal_idea_comment_inbox_event() returns trigger
language plpgsql security definer set search_path = '' as $$
begin
...
$$;
revoke all on function public.portal_idea_comment_inbox_event() from public, anon, authenticated, service_role;
drop trigger if exists portal_idea_comment_inbox_ain on public.idea_comments;
create trigger portal_idea_comment_inbox_ain after insert on public.idea_comments
  for each row execute function public.portal_idea_comment_inbox_event();

create or replace function public.portal_idea_comment_notify() returns trigger
language plpgsql security definer set search_path = '' as $$
declare
  v_recipient text;
  v_slug text;
...
$$;
revoke all on function public.portal_idea_comment_notify() from public, anon, authenticated, service_role;
drop trigger if exists portal_idea_comment_notify_ain on public.idea_comments;
create trigger portal_idea_comment_notify_ain after insert on public.idea_comments
...
create or replace function public.assert_portal_idea_comment_security()
...
begin
  ...
  select pg_catalog.pg_get_functiondef('public.add_idea_comment(uuid,text,uuid)'::pg_catalog.regprocedure) into v_def;
  if v_def is null or v_def not ilike '%security definer%' or v_def not ilike '%client_users%'
     or v_def not ilike '%portal_require_client_action%' or v_def not ilike '%portal_command_receipts%'
     or v_def not ilike '%portal_client_summary_shape_valid%' or v_def not ilike '%activity_log%' then
  ...
  select pg_catalog.pg_get_functiondef('public.add_agency_idea_comment_reply(uuid,text,text,uuid)'::pg_catalog.regprocedure) into v_def;
  if v_def is null or v_def not ilike '%security definer%' or v_def not ilike '%portal_feature_enabled%'
     or v_def not ilike '%agency_actors%' or v_def not ilike '%portal_command_receipts%'
  ...
  select pg_catalog.pg_get_functiondef('public.add_agency_idea_comment(uuid,text,text,uuid)'::pg_catalog.regprocedure) into v_def;
  if v_def is null or v_def not ilike '%security definer%' or v_def not ilike '%portal_feature_enabled%'
     or v_def not ilike '%agency_actors%' or v_def not ilike '%portal_command_receipts%'
  ...
  select pg_catalog.pg_get_functiondef('public.portal_idea_comment_inbox_event()'::pg_catalog.regprocedure) into v_def;
  ...
  select pg_catalog.pg_get_functiondef('public.portal_idea_comment_notify()'::pg_catalog.regprocedure) into v_def;
...
returns void language plpgsql security definer set search_path = '' as $$
begin
  perform public.assert_portal_slice39_security();
