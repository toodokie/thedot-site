...
- Production is deployed via **`vercel --prod`** from the working tree, so a deploy ships the **local working tree**, committed or not. ⚠️ **The repo IS GitHub-connected** (contrary to older notes): pushes to **feature branches create Vercel _preview_ deploys**, and the Vercel **production branch is `main`** — so **pushing `main` triggers a _production_ deploy** of whatever `main` contains. Production is normally shipped by running `vercel --prod` on top of that, not by pushing `main` (main is typically behind the deployed working tree).
...
- Route `/tools/ai-visibility` (`src/app/tools/ai-visibility/page.tsx`), API `src/app/api/ai-visibility/route.ts`, component `src/components/AiVisibilitySelfCheck.tsx`, shared prompts `src/lib/aivc-prompts.ts` (imported by BOTH API + component so **shown === run**).
- **Surfacing (lead engine — where the tool is linked from):**
  - **Homepage module** `src/components/AiCheckPromo.tsx` — a light contained "feature panel" (cream section → white panel + soft yellow glow + hairline; text left, CTA right) sitting in `HomePage.tsx` between `LatestFromJournal` and `ServicesSection`. **Keep it in the site's LIGHT language — no dark backgrounds (the site has never used one).** CTAs reuse the canonical **`.services-cta-button`** global class (white / ink border / **yellow glow shadow**, hover fills ink) — do NOT hand-roll a flat-yellow button; yellow is always a glow, never a fill.
  - **Footer "Free tools"** block (`Footer.tsx`) — site-wide, links the AI Visibility Check + The Hidden Revenue Finder (the homepage `ConversionDiagnosticTool`, anchor `#hidden-revenue-finder`).
  - **Blog markers** (handled in `BlogPostPage.tsx`, each optionally wrapped in `<p>` by the CMS): `[[ai-visibility-tool]]` embeds the **full interactive self-check**; `[[ai-visibility-cta]]` drops a **lightweight end-of-post CTA** (`src/components/AiCheckCta.tsx`) whose links carry **`?src=blog`** so analytics can see which posts drive checks. Use the CTA marker in posts that reference the tool but don't embed the whole widget; posts with neither marker render unchanged.
...
- **⚠️ Next `<Link>` + styled-jsx = silently unstyled** (cost us an invisible homepage CTA): styled-jsx scopes rules by stamping a `jsx-<hash>` class onto **native DOM elements** in the component, but a Next.js `<Link>` does **not** receive that scope class — so your scoped `.your-class { ... }` rules never match its `<a>`, and it renders with only the global `a` styles (underlined inline text, no background/padding). Fix: for a styled link/button inside a styled-jsx component, use a **native `<a href>`** (it gets scoped correctly; keep `onClick` for analytics), or apply the critical props as **inline `style={{}}`** (the pattern `LatestFromJournal` uses). Verify by checking the element's `className` includes the `jsx-…` scope class.
