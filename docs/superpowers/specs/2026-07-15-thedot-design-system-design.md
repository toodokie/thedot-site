...
packages/design-system/
  ...
  src/
    ...
    assets/    dot-motif.svg  shape-6-2.svg  shape-6-3.svg  shape-7-1.svg  arrow.svg  stripe.png
...
```css
:root{
  /* Canonical brand palette — the 5 */
  --dot-canvas:#faf9f6;  --dot-ink:#35332f;  --dot-yellow:#daff00;
  --dot-graphite:#47453f;  --dot-grey:#7a776f;
  --dot-yellow-pale:#eefb9d;            /* gradient midtone ONLY, never a flat fill */
  --dot-hairline:#ebebe7;               /* 1px borders (used instead of shadows) */

  /* Signature gradients (from production) */
...
| `Text` | `size:'lg'\|'md'\|'sm'`, `tone?:'ink'\|'grey'\|'graphite'`, `as?` | Real Text body |
| `Eyebrow` | `tone?`, `children` | uppercase Futura 600, wide tracking |
| `Button` | `variant:'ink'\|'yellow'\|'ghost'`, `size?:'md'\|'sm'`, `as?:'button'\|'a'` | sharp corners, uppercase, 2px border |
| `Card` | `eyebrow?`, `title?`, `children` | hairline border, flat (no shadow) |
| `Tag` | `tone:'yellow'\|'ink'`, `children` | uppercase pill |
| `Input` | `label?`, `invalid?`, native input props | ink focus, sharp |
...
| `Dot` | `fill:'silver'\|'ink'\|'yellow'`, `size?` | gradient-filled circle |
| `DotGrid` | `cols`, `rows`, `pattern?` | the poster device; deterministic ink/yellow/silver scatter |
...
- **Typekit reachability:** fonts load via Adobe's hosted `<link>`. If `/design-sync`'s screenshot
  ...
  in the conventions header; accept fallback in the sync render if Adobe licensing forbids self-hosting.
- **CSS Modules + tsup CSS bundling** is the fiddliest build step. Fallback: Vite library mode if
  tsup's CSS emission proves painful.
- **Workspaces:** adding `"workspaces"` to the root `package.json` is additive; verify `next build`
  and `npm ci` still succeed after the change.
- **`#47453f` (Graphite)** is a new canonical color (0 uses in current code) — intentional, per the
  approved palette.

## 9. Milestones (for the implementation plan to expand)

1. Workspace + package scaffold (`package.json` workspaces, `packages/design-system/` with tsup, tsconfig).
2. Tokens (`tokens.css`/`tokens.ts`), `fonts.css`, `reset.css`.
3. Assets: bring brand SVGs/stripe from `public/images` into `src/assets`.
